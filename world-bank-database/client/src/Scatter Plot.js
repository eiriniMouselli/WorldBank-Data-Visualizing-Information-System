...
class ScatterPlot extends Component {
  ...
  handleSubmit(event){
    ...
    if(checkedIndicators.length !== 2){
      alert("Please choose two indicators.");
      complete =false;
    }
    ...
    if(complete){
      ...
      .then(data => {
          ...
          data: {
              datasets: [{
                ...
                borderColor: "rgb(75,0,130)",
                  label: String(startY) +"-"+ String(endY),
                  data: [],
              }]
          },
          options: {
        ...
        for (var i =0; i<graphData[0].length; i++){
            window.bar.data.datasets[0].data.push({
                x: graphData[0][i],
                y: graphData[1][i]
            });
            window.bar.update(); 
  ...
  render() {
    return (
          <div class="nav"> 
            <div>
              <label>Select 1 country:</label>
              <label>Select 2 indicators:</label>
